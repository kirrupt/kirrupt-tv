defmodule KirruptTv.Helpers.FileHelpers do
  ...
  def download_file(url, dest_path) do
    if body = get_image(url) do
      case File.write(dest_path, body) do
        {:error, reason} ->
          Logger.error("Could not write file from #{url} -> File.write error :#{reason}")
          nil
  ...
  def file_exists(
        name,
        base_folder \\ "#{KirruptTv.Helpers.FileHelpers.root_folder()}/static",
        dest_folder \\ nil
      ) do
    image_path =
      case dest_folder do
  ...
  def mkdirs(folder_path) do
    if File.exists?(folder_path) do
      ...
      case File.mkdir_p(folder_path) do
        {:error, reason} ->
          Logger.error(
            "Could not create folders '#{folder_path}' -> File.mkdir_p error :#{reason}"
          )
