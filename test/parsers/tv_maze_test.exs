defmodule KirruptTv.TVMazeTest do
    ...
    test "parse valid year provided" do
        assert TVMaze.parse_year(%{:a => "b"}, %{"premiered" => "2020-12-01"})  == %{:a => "b", :year => 2020}
    end
    ...
    test "parse image provided" do
        assert TVMaze.parse_image(%{:a => "b"}, %{"image" => %{"original" => "path.jpg"}})  == %{:a => "b", :image => "path.jpg"}
    end
    ...
    test "parse network provided" do
        n = TVMaze.parse_network(%{:a => "b"}, %{"network" => %{"country" => %{"code" => "US", "timezone" => "Los Angeles"}}}) 
        assert n == %{:a => "b", :origin_country => "US", :timezone => "Los Angeles"}
    end

    test "parse network without code" do
        n = TVMaze.parse_network(%{:a => "b"}, %{"network" => %{"country" => %{"timezone" => "Los Angeles"}}}) 
    ...
    test "parse schedule provided" do
        s = TVMaze.parse_schedule(%{:a => "b"}, %{"schedule" => %{"time" => "10:00", "days" => ["Tuesday", "Monday"]}})
    ...
    test "parse embedded" do
        ...
        assert TVMaze.parse_embedded(%{:a => "b"}, data) == %{
            :a => "b",
            :episodes => [
                %{airdate: "airdate", episode: 2, screencap: "cap.jpg", season: 3, summary: "Some description", title: "Best EP ever", url: "some-url"},
                %{airdate: "airdate", episode: 4, screencap: "cap.jpg", season: 5, summary: "Some description", title: "Best EP ever", url: "some-url"}
            ]
        }
    end
        ...
        assert show == %{
            ...
            episodes: [
              ...
              %{
                airdate: "2014-03-26",
                episode: 10,
                screencap: "http://static.tvmaze.com/uploads/images/original_untouched/145/363950.jpg",
                season: 8,
                summary: "Shawn and Gus assist Lassiter and Betsy Brannigan, with the help of Henry and Woody, in solving the murder of real estate executive, Warren Dern. Meanwhile, Shawn has decided to move to San Francisco to be with Juliet, but is struggling to find a way to break the news to Gus.",
                title: "The Break Up",
