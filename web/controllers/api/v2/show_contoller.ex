defmodule KirruptTv.Api.V2.ShowController do
    ...
    ignored_show_ids =
      cond do
        conn.assigns[:current_user] ->
          Model.User.get_user_ignored_show_ids(conn.assigns[:current_user])
  ...
  def updated_dates(conn, %{"show_ids" => show_ids}) do
    render(conn, "updated_dates.json",
      data: %{
        shows: Model.Show.get_show_updated_dates(show_ids),
        episodes: Model.Show.get_show_episodes_updated_dates(show_ids)
      }
    )
  end

  def sync_ignored(conn, %{"shows" => shows}) do
    render(conn, "sync_ignored.json",
      data: %{user_shows: Model.User.sync_ignored(conn.assigns[:current_user], shows)}
    )
  end
  ...
  def update_any_show(conn, _params) do
    case Model.Show.update_any_show() do
      ...
      id ->
        render(conn, "update_any_show.json",
          data: %{show: Repo.get(Model.Show, id) |> Repo.preload([:genres])}
        )
    end
  end
end
