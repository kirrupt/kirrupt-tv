defmodule Model.Episode do
  ...
  def insert_or_update(show, e_data) do
    episode =
      case Repo.get_by(Model.Episode, %{
             season: e_data[:season],
             episode: e_data[:episode],
             show_id: show.id
           }) do
        nil ->
          %Model.Episode{season: e_data[:season], episode: e_data[:episode], show_id: show.id}
    ...
    case result do
      ...
      {:error, _changeset} ->
        Logger.error(
          "Could't insert or update episode '#{e_data[:episode]}' for show id #{show.id}"
        )
