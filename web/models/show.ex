defmodule Model.Show do
  ...
  def next_episode(show_obj) do
    Repo.all(
      from(e in Episode,
        where:
          e.show_id == ^show_obj.id and e.airdate >= ^Timex.today() and
            fragment("?", e.airdate) != ^"2013-00-00 00:00:00",
  ...
  defp watched_episodes(show_obj, user) do
    Repo.all(
      from(e in Episode,
        join: w in Model.WatchedEpisode,
        on: w.episode_id == e.id and w.user_id == ^user.id,
        where: e.show_id == ^show_obj.id,
        select: e.id
      )
    )
  ...
  defp show_episodes(show_obj, season) do
    Repo.all(
      ...
      |> where(
        [e, s],
        e.airdate < ^Timex.today() and fragment("?", e.airdate) != ^"2013-00-00 00:00:00" and
          e.show_id == ^show_obj.id
      )
      |> season_query(season)
  ...
  def search(q, user) do
    ...
    Repo.all(
      from(s in Model.Show,
        left_join: us in Model.UserShow,
        on: us.show_id == s.id and us.user_id == ^user_id,
  ...
  def update_tvmaze_show_and_episodes(s_obj, info) do
    ...
    info[:genres]
    |> Enum.each(fn genre ->
      if !Enum.find(s_obj.genres, fn s_genre ->
           String.downcase(s_genre.name) == String.downcase(genre)
         end) do
    ...
    s_obj =
      case result do
        ...
        {:error, changeset} ->
          Logger.error("Could't update show '#{s_obj.id}' - #{inspect(changeset.errors)}")
          fake_update(s_obj)
          false
      end
  ...
  def update_show_and_episodes(s_obj) do
    case KirruptTv.Parser.TVMaze.show_info(s_obj.tvmaze_id) do
      nil ->
        Logger.error("Could't update show '#{s_obj.id}'")
        fake_update(s_obj)
        nil

      info ->
        update_tvmaze_show_and_episodes(s_obj, info)
    end
  end
  ...
  defp fake_update(show) do
    ...
    |> case do
      ...
      {:error, changeset} ->
        Logger.error("Couldn't fake update show '#{show.id}' - #{inspect(changeset.errors)}")
        false
    end
  end
  ...
  def set_fanart_tv_image(s_obj) do
    cond do
      images = KirruptTv.Parser.FanartTV.get_image_list(s_obj.thetvdb_id) ->
        ...
        changes =
          get_fanart_image_changes(changes, s_obj, images, :fixed_background, :showbackground)
        ...
        case result do
          ...
          {:error, _changeset} ->
            Logger.error("Could't update show images '#{s_obj.id}'")
            s_obj
        end
    ...
    case s_obj.url do
      nil ->
        ...
        case result do
          ...
          {:error, _changeset} ->
            Logger.error("Could't update show url '#{s_obj.id}'")
            s_obj
        end
